...
function startSocketServer(httpServer) {
  ...
  io.use((socket, next) => {
    ...
    try {
      ...
      console.log('[Socket.IO] Token verified, userId:', payload.id);
  ...
  async function sendChatsListTo(userId) {
    try {
      ...
      const results = await Promise.all(chats.map(async (c) => {
        ...
        const unread = await Message.countDocuments({ chatId: c._id, senderId: { $ne: userId }, status: { $ne: 'read' } });
        return {
          chatId: c._id,
  ...
  io.on('connection', socket => {
    ...
    sendChatsListTo(userId).catch(err => console.warn('initial chats_list send failed', err));
    ...
    socket.on('open_chat', async ({ otherUserId }, ack) => {
      try {
        ...
        try {
          await Message.updateMany({ chatId, senderId: { $ne: userId }, status: { $ne: 'read' } }, { $set: { status: 'read' } });
        } catch (markErr) {
        ...
        ack && ack({ ok: true, chatId: chat._id, participants: chat.participants });
      } catch (err) {
    ...
    socket.on('send_message', async ({ chatId, text }, ack) => {
      try {
        if (!chatId || !text) return ack && ack({ ok: false, error: 'invalid_payload' });

        const chat = await Chat.findById(chatId);
        if (!chat) return ack && ack({ ok: false, error: 'chat_not_found' });

        const isParticipant = chat.participants.some(p => p.userId === userId);
        if (!isParticipant) return ack && ack({ ok: false, error: 'not_allowed' });
        ...
        const senderPart = chat.participants.find(p => p.userId === userId) || {};
        const senderName = senderPart.displayName || 'Anonymous';
        const payload = { _id: msg._id, chatId, senderId: userId, senderName, text, createdAt: msg.createdAt };
        ...
        try {
          const recipients = chat.participants.filter(p => p.userId !== userId).map(p => p.userId);
          recipients.forEach(rid => {
            io.to(`user_${rid}`).emit('incoming_message', { chatId, message: payload, from: userId });
          });
        } catch (notifyErr) {
    ...
    socket.on('join_chat', ({ chatId }, ack) => {
      if (chatId) { socket.join(`chat_${chatId}`); ack && ack({ ok: true }); }
